...
//     } finally {
//       setLoading(false);
//       setIsProcessing(false); // End processing state after submission
//     }
//   };
...
//           onClose={() => {
//             setIsImageModalOpen(false);
//             setIsProcessing(false); // Reset processing state when closing
//           }}
//         >
//           {loading ? (
...
//               </p>
//               <ClipLoader size={40} color="black" />
//               <QrCodeButton onClick={handleScanSubmit} disabled={loading}>
//                 Submit
//               </QrCodeButton>
//             </div>
//           ) : qrCodeData ? (
...
//           // Extract QR code from image (client-side only)
//           const qrData = await extractQRCode(imageData);
//           setLoading(false);
...
//         >
//           {loading ? (
//             // Show loading state while extracting QR code from image
//             <div style={{ textAlign: "center", padding: "10px" }}>
//               <p style={{ fontWeight: "bold", marginBottom: "5px" }}>
...
//             </div>
//           ) : qrCodeData ? (
//             // Show extracted QR code data and submit button
//             <div style={{ textAlign: "center", padding: "10px" }}>
...
//               <QrCodeData>{qrCodeData}</QrCodeData>
//               {isProcessing ? (
//                 // Show processing state when submitting to API
...
//             </div>
//           ) : (
//             // Show the uploaded image if no QR code was found
//             selectedImage && (
...
//       // Strategy 2: Apply multiple preprocessing techniques
...
//       // Strategy 4: Try scaling the image
...
//   // Adjust contrast
//   const factor = (259 * (128 + 50)) / (255 * (259 - 50)); // Increase contrast
//   for (let i = 0; i < data.length; i += 4) {
//     data[i] = clamp(factor * (data[i] - avg) + avg); // Red
//     data[i + 1] = clamp(factor * (data[i + 1] - avg) + avg); // Green
//     data[i + 2] = clamp(factor * (data[i + 2] - avg) + avg); // Blue
//   }
...
//       outputData[idx + 3] = inputData[idx + 3]; // Alpha
//     }
//   }
// };
...
const Header: React.FC = () => {
  ...
  const setupCamera = async () => {
    try {
      ...
      setIsScanning(false);
      toast.error(
        "Failed to access back camera. Please check permissions or try again."
      );
    }
  };
